...
import { getMethodColor } from "./bruParser";
...
export class BruQuickOpenModal extends FuzzySuggestModal<RunnableRequestFile> {
  ...
  renderSuggestion(match: FuzzyMatch<RunnableRequestFile>, el: HTMLElement): void {
    ...
    const badge = top.createEl("span", { text: item.method, cls: "bru-method-badge bru-qo-badge" });
    badge.style.background = getMethodColor(item.method);
    top.createEl("span", { text: item.name, cls: "bru-qo-name" });
